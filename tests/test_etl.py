...
def test_relevant_policies_view():
    ...
    for actual, expected in zip(actual_rows, expected_rows):
        ...
        assert actual[:4] == expected[:4], f"Mismatch in row: {actual[:4]} != {expected[:4]}"
        # Compare avg_days_since_update rounded to nearest int
        assert round(actual[4]) == expected[4], f"Expected avg_days_since_update ≈ {expected[4]}, got {actual[4]}"
